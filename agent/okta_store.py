...
"""
import hashlib, json, os, re, time, pathlib, datetime as dt
...
def handle(file, method, url, body=None):
    ...
    if method == "POST" and path == "/api/v1/users":
        ...
        # sha256, not hash(): hash() is randomised per process, so the same
        # person staged twice used to become two different people. The HTTP
        # server in services/mock_okta/app.py derives the id identically, so
        # both backends agree on who someone is.
